 ...
 */
function expandBrTags(value: string | null | undefined): ElementContent[] {
	const safeValue = typeof value === 'string' ? value : value == null ? '' : String(value);
	const matches = [...safeValue.matchAll(BR_PATTERN)];
	if (!matches.length) return [{ type: 'text', value: safeValue } as Text];
	...
	for (const m of matches) {
		if (m.index! > cursor) {
			result.push({ type: 'text', value: safeValue.slice(cursor, m.index) } as Text);
		}
	...
	if (cursor < safeValue.length) {
		result.push({ type: 'text', value: safeValue.slice(cursor) } as Text);
	}
 ...
 */
function parseList(value: string | null | undefined): Element | null {
	const safeValue = typeof value === 'string' ? value : value == null ? '' : String(value);
	const match = safeValue.trim().match(LIST_PATTERN);
