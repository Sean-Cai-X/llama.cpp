...
class ConversationsStore {
	...
	async findConversationBySessionId(sessionId: string): Promise<DatabaseConversation | null> {
		return (await DatabaseService.getConversationBySessionId(sessionId)) ?? null;
	}

	async restoreConversationBySessionId(sessionId: string): Promise<boolean> {
	...
	async getMessagesBySessionId(sessionId: string): Promise<DatabaseMessage[]> {
		return DatabaseService.getMessagesBySessionId(sessionId);
	}

	async getMessagesByTaskId(taskId: string): Promise<DatabaseMessage[]> {
		return DatabaseService.getMessagesByTaskId(taskId);
	}

	async getMessagesByIntentKey(intentKey: string): Promise<DatabaseMessage[]> {
		return DatabaseService.getMessagesByIntentKey(intentKey);
	}
