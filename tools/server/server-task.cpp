...
static json structured_conclusion_to_json(
        const task_params & generation_params,
...
json server_task_result_cmpl_final::to_json_oaicompat() {
    ...
    res["structured_conclusion"] = structured_conclusion_to_json(generation_params, content, false);
...
json server_task_result_cmpl_final::to_json_oaicompat_chat() {
    ...
    res["structured_conclusion"] = structured_conclusion_to_json(generation_params, msg.content, false);
...
json server_task_result_cmpl_final::to_json_oaicompat_chat_stream() {
    ...
    if (!deltas.empty()) {
        deltas.front()["structured_conclusion"] = structured_conclusion_to_json(generation_params, oaicompat_msg.content, false);
    }
...
static json structured_conclusion_to_json(
        ...
        const std::string & content,
        bool is_partial) {
    json result = generation_params.structured_conclusion_input.is_object()
        ? generation_params.structured_conclusion_input
        : json::object();

    const auto get_string = [&](const std::string & key, const std::string & fallback = std::string()) {
        if (!result.contains(key) || result.at(key).is_null()) {
            return fallback;
        }
        if (result.at(key).is_string()) {
            return result.at(key).get<std::string>();
        }
        return result.at(key).dump();
    };

    const auto ensure_array = [&](const std::string & key) {
        if (!result.contains(key) || result.at(key).is_null()) {
            result[key] = json::array();
        } else if (!result.at(key).is_array()) {
            result[key] = json::array({ result.at(key) });
        }
    };
    ...
    const auto primary_intent = get_string("primary_intent", "unresolved");
    const double intent_confidence = result.contains("intent_confidence") && result.at("intent_confidence").is_number()
        ? result.at("intent_confidence").get<double>()
        : 0.0;

    ensure_array("secondary_intents");
    ensure_array("entity_refs");
    ...
    const bool insufficient_context = missing_primary_intent || intent_confidence < 0.35 || missing_evidence;

    result["task_state"] = get_string("task_state", "observe");
    result["reasoning_level"] = get_string("reasoning_level", "medium");
    result["primary_intent"] = primary_intent;
    result["intent_confidence"] = intent_confidence;
    result["association_scope"] = get_string("association_scope");
    ...
    result["insufficient_context"] = insufficient_context;
    result["producer"] = "llama.cpp-b8851";
    result["stage"] = is_partial ? "partial" : "final";
...
json server_task_result_cmpl_partial::to_json_non_oaicompat() {
    ...
    res["structured_conclusion"] = structured_conclusion_to_json(generation_params, content, true);
    return res;
}
...
json server_task_result_cmpl_partial::to_json_oaicompat_chat() {
    ...
    if (!deltas.empty()) {
        deltas[0]["structured_conclusion"] = structured_conclusion_to_json(generation_params, content, true);
    }
